%% This example illustrates how the Toolkit could be used to determine the lowest dose of chlorine applied at the entrance to a distribution system needed to ensure that a minimum residual is met throughout the system.
% 
% 
...
% the minimum residual target is given by Ctarget, and the target is only checked 
% after a start-up duration of 5 days (432,000 seconds). To keep the code more 
% readable, no error checking is made on the results returned from the Toolkit 
% function calls.
%% <https://github.com/OpenWaterAnalytics/EPANET/wiki/Example-3 https://github.com/OpenWaterAnalytics/EPANET/wiki/Example-3>
%% 
%% Clear - Start Toolkit 
...
while (csource <= 4.0)
    ...
    while (~violation && (tstep > 0))
        ...
        if (t > 432000)
            c = d.getNodeActualQuality();   
            violation = any(c < Ctarget);
        end
