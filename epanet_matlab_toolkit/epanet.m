classdef epanet <handle
    ...
    methods (Static)
        ...
        function [Errcode, value] = apiENgetstatistic(code, LibEPANET)
            ...
            % EPANET 20100
            %
            % apiENgetstatistic(code, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode, count] = apiENgetcount(countcode, LibEPANET)
            ...
            %
            % apiENgetcount(countcode, LibEPANET)
            %
            % Parameters:
          ...
          function [errmsg, e] = apiENgeterror(Errcode, LibEPANET)
            ...
            %
            % apiENgeterror(Errcode, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode, value] = apiENgetoption(optioncode, LibEPANET)
            ...
            %
            % apiENgetoption(optioncode, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode, timevalue] = apiENgettimeparam(paramcode, LibEPANET)
            ...
            %
            % apiENgettimeparam(paramcode, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode] = apiENinit(unitsType, headLossType, LibEPANET)
            ...
            % EPANET Version 2.2
            %
            % apiENinit(unitsType, headLossType, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode] = apiENcopyreport(filename, LibEPANET)
            ...
            % EPANET Version 2.2
            %
            % apiENcopyreport(filename, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode, value] = apiENgetresultindex(objecttype, index, LibEPANET)
            ...
            % EPANET Version 2.2
            %
            % apiENgetresultindex(objecttype, index, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode] = apiENsaveHydfile(fname, LibEPANET)
            ...
            %
            % apiENsaveHydfile(fname, LibEPANET)
            %
            % Parameters:
            % LibEPANET epanet library DLL name
            %
        ...
        function [Errcode] = apiENsaveinpfile(inpname, LibEPANET)
            ...
            %
            % apiENsaveinpfile(inpname, LibEPANET)
            %
            % Parameters:
            % LibEPANET epanet library DLL name
            %
        ...
        function [Errcode] = apiENsetoption(optioncode, value, LibEPANET)
            ...
            %
            % apiENsetoption(optioncode, value, LibEPANET)
            %
            % Parameters:
            % optioncode   a type of analysis option (see EN_Option).
        ...
        function [Errcode] = apiENsetqualtype(qualcode, chemname, chemunits, tracenode, LibEPANET)
            ...
            % LibEPANET   the ID name of the node being traced if qualType = EN_TRACE.
            %
            % Returns:
            % an error code.
            % OWA-EPANET Toolkit: http://wateranalytics.org/EPANET/group___options.html
        ...
        function [Errcode] = apiENsetreport(command, LibEPANET)
            ...
            %
            % apiENsetreport(command, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode] = apiENsetstatusreport(statuslevel, LibEPANET)
            ...
            %
            % apiENsetstatusreport(statuslevel, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode] = apiENsettimeparam(paramcode, timevalue, LibEPANET)
            ...
            %
            % apiENsettimeparam(paramcode, timevalue, LibEPANET)
            %
            % Parameters:
            % paramcode    a time parameter code (see EN_TimeParameter).
        ...
        function [Errcode] = apiENusehydfile(hydfname, LibEPANET)
            ...
            %
            % apiENusehydfile(hydfname, LibEPANET)
            %
            % Parameters:
        ...
        function [index, Errcode] = apiENaddnode(nodeid, nodetype, LibEPANET)
            ...
            %
            % apiENaddnode(nodeid, nodetype, LibEPANET)
            %
            % Parameters:
        ...
        function [Errcode] = apiENsettitle(line1, line2, line3, LibEPANET)
            ...
            % EPANET Version 2.2
            %
            % apiENsettitle(line1, line2, line3, LibEPANET)
            %
            % Parameters:
            % line1   first title line
            % line2   second title line
        ...
        function [Errcode] = apiENsetflowunits(code, LibEPANET)
            ...
            %
            % apiENsetflowunits(code, LibEPANET)
            %
            % Parameters:
    ...
    methods
        ...
        function value = getRuleID(obj, varargin)
            ...
            for i=1:length(index)
                [obj.Errcode, value{j}] = obj.apiENgetruleID(index(i), obj.LibEPANET);
                error(obj.getError(obj.Errcode));
                j=j+1;
            end
        end
        function value = getRuleInfo(obj, varargin)
            ...
            value.Index = index;
            for i=1:length(index)
                [~, value.Premises(i), value.ThenActions(i), value.ElseActions(i), value.Priority(i)] = obj.apiENgetrule(index(i), obj.LibEPANET);
                error(obj.getError(obj.Errcode));
            end
        end
        ...
        function value = getLinkQuality(obj, varargin)
            % Retrieves the value of link quality.
            % Pipe quality
            %
            % Example 1:
            ...
            % See also getLinkType, getLinksInfo, getLinkDiameter,
            %          getLinkRoughnessCoeff, getLinkMinorLossCoeff.
            value = get_link_info(obj, obj.ToolkitConstants.EN_QUALITY, varargin{:});
        end
        ...
        function index = addNodeJunction(obj, juncID, varargin)
            ...
            [index, errcode] = obj.apiENaddnode(juncID, obj.ToolkitConstants.EN_JUNCTION, obj.LibEPANET);
            error(obj.getError(errcode));
        ...
        function index = addNodeTank(obj, tankID, varargin)
            ...
            [index, errcode] = obj.apiENaddnode(tankID, obj.ToolkitConstants.EN_TANK, obj.LibEPANET);
            error(obj.getError(errcode));
            obj.setNodeCoordinates(index, [xy(1), xy(2)]);
        ...
        function [leftPipeIndex ,rightPipeIndex] = splitPipe(obj, pipeID, newPipeID, newNodeID)
            ...
            %
            % Example 1:
            %   Splits pipe with ID '11' to pipes '11' and '11a' and creates the
            ...
            %
            % Example 2:
            %   Splits pipe with ID 'P-837' to pipes 'P-837' and 'P-837a' and creates the
            %   node 'P-837node' in the link of the two new pipes, using
            %   vertices
            %   d = epanet('ky10.inp');
            %   pipeID = 'P-837';
            %   newPipeID= 'P-837a';
            ...
            if (coordNode1(1) == 0 && coordNode1(2) == 0 ...
                || coordNode2(1) == 0 && coordNode2 == 0 )
                error('Some nodes have zero values for coordinates')
            end
            if (isempty(obj.getLinkVertices{pipeIndex}))
                % Calculate mid position of the link/pipe based on nodes
                midX = (coordNode1(1)+coordNode2(1))/2;
                midY = (coordNode1(2)+coordNode2(2))/2;
            else
                % Calculate mid position based on vertices
                % pick midpoint of vertices
                xVert = obj.getLinkVertices{pipeIndex}.x;
                xMidPos  = floor(length(xVert)/2);
                midX = obj.getLinkVertices{pipeIndex}.x(xMidPos);
                midY = obj.getLinkVertices{pipeIndex}.y(xMidPos);
            end
       ...
       function setNodeTankCanOverFlow(obj, value, varargin)
        ...
        function setOptionsHeadLossFormula(obj, value)
            ...
            if strcmpi(value, 'HW')
                codevalue=0;
            elseif strcmpi(value, 'DW')
                codevalue=1;
            elseif strcmpi(value, 'CM')
                codevalue=2;
            end
        ...
        function value = getMSXPatternsLengths(obj, varargin)
            ...
            elseif isa(varargin{1}, 'numeric')
                k=1;
                for i=varargin{1}
                    [obj.Errcode, value(k)]=obj.apiMSXgetpatternlen(i, obj.MSXLibEPANET);
                    if obj.Errcode, error(obj.getMSXError(obj.Errcode)); end
                    k=k+1;
                end
            end
        ...
        function setMSXSources(obj, nodeID, speciesID, sourcetype, concentration, patID)
            ...
            %   d.setMSXSources(d.NodeNameID{2}, d.MSXSpeciesNameID{1}, ...
            %                       'Setpoint', 0.5, 'PatAsIII') % Sets the second node as setpoint.
            %   srcs = d.getMSXSources
            ...
            obj.apiMSXsetsource(node, species, type, concentration, pat, obj.MSXLibEPANET);
        end
        ...
        function setMSXParametersTanksValue(obj, NodeTankIndex, paramindex, value)
            ...
            [obj.Errcode] = obj.apiMSXsetparameter(0, NodeTankIndex, paramindex, value, obj.MSXLibEPANET);
            if obj.Errcode, error(obj.getMSXError(obj.Errcode)); end
        end
        ...
        function setMSXNodeInitqualValue(obj, value)
            ...
            %   d.setMSXNodeInitqualValue(values) % Sets all values and change the first node's value to 1500.
            %   d.getMSXNodeInitqualValue  
            for i=1:length(value)
                for j=1:size(value{1}, 2)
                    [obj.Errcode] = obj.apiMSXsetinitqual(0, i, j, value{i}(j), obj.MSXLibEPANET);
                    if obj.Errcode, error(obj.getMSXError(obj.Errcode)); end
                end
            end
        end
        function setMSXLinkInitqualValue(obj, value)
            ...
            %   values{linkIndex}(speciesIndex)=1500;
            %   d.setMSXLinkInitqualValue(values) % Sets all values and change the first link's value to 1500.
            %   d.getMSXLinkInitqualValue  
            for i=1:length(value)
                for j=1:size(value{1}, 2)
                    [obj.Errcode] = obj.apiMSXsetinitqual(1, i, j, value{i}(j), obj.MSXLibEPANET);
                    if obj.Errcode, error(obj.getMSXError(obj.Errcode)); end
                end
            end
        end
        ...
        function setMSXPatternMatrix(obj, patternMatrix)
            ...
            nfactors=size(patternMatrix, 2);
            for i=1:size(patternMatrix, 1)
                [obj.Errcode] = obj.apiMSXsetpattern(i, patternMatrix(i, :), nfactors, obj.MSXLibEPANET);
                if obj.Errcode, error(obj.getMSXError(obj.Errcode)); end
            end
        end
        function setMSXPatternValue(obj, index, patternTimeStep, patternFactor)
            ...
            %   d.setMSXPatternValue(1,1,0.0); % Sets the first timestep of the first pattern to 0.
            %   d.getMSXPattern  
            [obj.Errcode] = obj.apiMSXsetpatternvalue(index, patternTimeStep, patternFactor, obj.MSXLibEPANET);
            if obj.Errcode, error(obj.getMSXError(obj.Errcode)); end
        end
        ...
        function initializeMSXQualityAnalysis(obj, flag)
            ...
            %   d.solveMSXCompleteHydraulics;
            %   while(tleft>0)
            %       [~, tleft] = d.stepMSXQualityAnalysisTimeLeft;
            %   end
            %
            % See also
